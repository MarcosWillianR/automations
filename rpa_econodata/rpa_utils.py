...
class WebScraperUtils:
    def __init__(self):
        self.file = 'empresas.xlsx'

    def price_format(self, texto):
        if texto == "Indisponível":
            return None
        
        if type(texto) is float:
            return texto
    ...
    def xlsx_update(self, data):
        if os.path.exists(self.file):
            ...
            df = pd.DataFrame(columns=['empresa', 'cnpj', 'capital_social'])

        for item in data:
            item['capital_social'] = self.price_format(item['capital_social'])
